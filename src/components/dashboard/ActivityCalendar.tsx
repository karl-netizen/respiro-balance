                  ...
                  <Tooltip key={dayIndex}>
                    <TooltipTrigger asChild>
                      <div
                        ...
                        aria-label={`${format(parseISO(day.date), 'MMM d')}: ${day.value} minutes`}
                      />
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="font-medium">{format(parseISO(day.date), 'MMM d')}</p>
...
function groupByWeek(entries: ActivityEntry[]): ActivityEntry[][] {
  ...
  entries.forEach((entry, index) => {
    const dayOfWeek = parseISO(entry.date).getDay();
